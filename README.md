...
## План спринтов (драфт)
**Спринт 5**
[x] Настроен монорепозиторий (`client/` + `server/`).
- Клиент:
  [x] Сверстаны ключевые страницы.
  [x] Настроен роутинг
  [x] Реализован драфт игры
  [x] Реализована логика авторизации и валидации
  [x] Исправлены ошибки и настроен ErrorBoundary

**Спринт 6 (текущий): игровое ядро, авторизация**
- 6.1 Реализована одна из заявленных игровых механик (Кирилл)
- 6.2 Реализованы состояния «Начало» и «Завершение игры» (Артур)
- 6.3 Проработана визуальная часть игры (Анна + Кирилл)
- 6.4 Добавлены Service Workers (Анна)
- 6.5 Авторизация проверяется через отдельный Hook / HOC (Сергей)
- 6.6 Добавлен минимум один Web API (Кирилл)
- 6.7 Добавлено Redux-хранилище (Антон)
- 6.8 Написаны тесты на игровой движок + любой UI (Сергей)
- 6.9 Получен командный зачёт (все)

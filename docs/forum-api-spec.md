...
| Q3 Порт для API | **'VITE_APP_API_URL'** на клиенте (fallback 'http://localhost:3000'); **'SERVER_PORT'** по умолчанию **3000** у 'packages/server'. В браузере форум — same-origin через SSR (**9000** + прокси). В Docker — разные опубликованные порты (см. '.env.sample', `CLIENT_PORT`). |

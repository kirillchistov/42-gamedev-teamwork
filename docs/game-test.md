...
> Примечание по ветке: тестовые файлы из `packages/client/src/**/*.test.*` и вспомогательные моки были сознательно удалены из этой ветки, чтобы не смешивать работу разных участников. Актуальные заготовки/примеры оставлены в этом документе.

## Известный нюанс Jest

При прямом запуске `GamePage.test.tsx` падение вида `Cannot find module ...png?url` связано не с логикой страницы, а с тем, что Jest по умолчанию не резолвит Vite-импорты ассетов с суффиксом `?url`.

Чтобы такой тест проходил в будущем, нужно заранее добавить в `jest.config.js` `moduleNameMapper` для ассетов (или отдельный мок для путей `?url`) и, при необходимости, декларации типов ассетов для TypeScript.

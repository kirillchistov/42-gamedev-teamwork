# Express и серверный рендеринг React в этом репозитории

Документ описывает **фактическое состояние** монорепозитория и даёт **конкретные врезки кода**, чтобы закрыть учебное задание по Express SSR (включая статические страницы без Redux), не ломая принятую структуру пакетов.

## 1. Карта проекта (что уже есть)
...
SSR в этом проекте **не живёт в `packages/server`**, а реализован **в клиентском пакете**: собранный серверный бандл лежит в `packages/client/dist/server/`, статика клиента — в `packages/client/dist/client/`. Так задумано: один процесс отдаёт HTML и ассеты SPA.

Зависимости для задания (Express, `react`, `react-dom`, Vite для dev-режима SSR) **уже перечислены** в `packages/client/package.json`. Отдельно ставить Express в корень репозитория не требуется, если вы работаете в рамках существующей схемы.

**Менеджер пакетов:** в корневом `package.json` указан **Yarn 1** (`packageManager: "yarn@1.22.22"`). Установка из корня: `yarn`.

---

## 2. Соответствие шагам задания

### 2.1. Установка зависимостей

- **Node.js:** версия из `engines` корня — `>=15` (для Vite 4 и TypeScript 6 лучше использовать актуальный LTS, например 20.x).
...
Маршрут «главной» в учебном смысле здесь реализован как **catch-all** `app.get('*', …)`: любой URL отдаётся через один SSR-обработчик (подходит для SPA с роутингом на клиенте).
...
Сейчас `entry-server.tsx` **всегда** оборачивает дерево в `<Provider store={…}>` и использует данные из Redux (`configureStore`, `fetchData`, сериализация `initialState` в HTML). Это **полнофункциональное** приложение, а не минимальный учебный пример.

Чтобы формально выполнить требование «статические React-страницы **без** Redux», не ломая остальную структуру, разумны два пути:

1. **Учебный минимум (отдельный маленький пример)** — отдельный файл рендера только для демо-маршрута (см. раздел 5). Тогда нужно согласовать **тот же** React-узел на клиенте при гидратации (`main.tsx`), иначе будет рассинхрон.
2. **Прагматично для этого репозитория** — считать задание выполненным на существующей связке Express + `renderToString` + статика, а «без Redux» описать как **возможное упрощение** дерева в `render` для отдельных путей (ветвление по `req.path`).
...
Добавьте **после** `app.get('*', async …)` и **до** `app.listen`:
...
## 5. Врезка: пример «статической» страницы без Redux (учебный шаблон)
...
**Шаг A — расширить** `packages/client/tsconfig.server.json`:

- В `compilerOptions` добавьте **`"jsx": "react-jsx"`**, иначе `tsc` не соберёт `.tsx` в каталоге `server/`.
- В массив `include` добавьте файл компонента:
...
(остальные поля `compilerOptions` оставьте как в репозитории; в примере показаны только нужные для JSX строки.)
...
- Для курса такой маршрут демонстрирует **`renderToString` + `res.send`/`type`** без Redux и без общего роутера.
...
После добавления врезок из разделов 4–5 перезапустите dev-сервер клиента и проверьте `/__ssr_static_demo` и основные маршруты приложения.

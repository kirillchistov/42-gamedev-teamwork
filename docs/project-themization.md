...
- В продукте **три** варианта оформления лендинга и общих страниц: `light-flat`, `light-3d`, `dark-neon` - две светлые плюс одна тёмная; переключатель «луна/солнце» на `/game` переводит между **тёмной** и **последней выбранной светлой** (flat или 3d).
...
**Итог по планированию:** **done**; при появлении новых макетов достаточно расширить тип `LandingTheme`, ключи `localStorage` и блоки в `themes.pcss` + точечные оверлеи в модульных `*.pcss`.
...
Рекомендация: не дублировать палитру в JS, если достаточно CSS; переменные (`:root` / модификаторы `.landing--*`) упростят жизнь.

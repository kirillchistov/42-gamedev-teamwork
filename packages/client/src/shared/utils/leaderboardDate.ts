...
export function normalizeLeaderboardRecordDate(raw: string): string {
...
export function leaderboardRecordDateToday(): string {
  return normalizeLeaderboardRecordDate(new Date().toISOString().split('T')[0])
}

/** Отображение в UI: dd.MM.yyyy (ru-RU) из нормализованной ISO-даты. */
/** Сравнение дат рекорда для сортировки (пустые — в конец). */
export function compareLeaderboardRecordDates(a: string, b: string): number {
...
export function formatLeaderboardRecordDateForDisplay(isoDate: string): string {
  const normalized = normalizeLeaderboardRecordDate(isoDate)
  ...
  return new Date(y, month - 1, day).toLocaleDateString('ru-RU', {
    day: '2-digit',
    month: '2-digit',
    year: 'numeric',
  })
